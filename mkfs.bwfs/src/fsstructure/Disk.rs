...
Salidas: El inode que se buscaba o Nada.
*/
pub fn find_inode_by_name(&self, parent_inode_ino: u64, name: &str) -> Option<&Inode> {
        for i in 0..self.inodes_block.len() {
...
Salidas: No tiene salidas.
*/
pub fn fill_memory_block(&mut self, ino:u64, data:u8) {
...
pub fn write_content(&mut self, ino_ref: u64, content: Vec<u8>) {
        for i in 0..content.len(){
            self.fill_memory_block(ino_ref.clone(), content[i].clone());
